...
do 
	jobid=`sbatch --parsable benchmarkscripts/svdd_cpu_ear.sh ${zdim} "${layers}" ${ft} ${iterations} ${batchsize} ${SVDDname}.h5 ${precision}`  # first dim then hidden layers of size then fixed target then iteration
